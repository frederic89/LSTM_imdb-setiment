...
def prepare_data(seqs, labels, maxlen=None):
    """Create the matrices from the datasets. 把数据转化为矩阵
    ...
    for idx, s in enumerate(seqs):
        x[:lengths[idx], idx] = s  # 相当于x[0:lengths[idx], idx] = s ,按列纵向填充s
...
def load_data(path="imdb.pkl", n_words=100000, valid_portion=0.1, maxlen=None,
    ...
    if maxlen:
        new_train_set_x = []
        new_train_set_y = []
        for x, y in zip(train_set[0], train_set[1]): #准备语料中的命令pkl.dump((test_x, test_y), f, -1)
            if len(x) < maxlen:
